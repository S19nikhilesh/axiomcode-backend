...
const submitCode = async (req, res) => {
    try {
        ...
        // 🎯 TERA EXISTING IF-BLOCK (Yahan bina chedkhani kiye POTD handle kiya hai)
        if (!req.result.ProblemSolved.includes(problemId)) {
            req.result.ProblemSolved.push(problemId);
        }

        // ==========================================
        // 🚀 APNA SEPARATE POTD HANDLE (SAFE ZONE)
        if (status === 'accepted') {
            const todayStr = new Date().toISOString().split('T')[0];
            const activePotd = await PotdStore.findOne({ dateString: todayStr });

            // Check agar yeh aaj ka POTD hai
            if (activePotd && activePotd.problemId.toString() === problemId.toString()) {
                ...
                // Agar aaj pehle solve nahi kiya, tabhi fields update karo
        ...
        // Ek hi baar final save user document ke liye
